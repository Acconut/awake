...
// Ensure a make target is called, multiple targets are collected into a single
// invocation of `make`
var targets = null;
function make(target) {
    if (targets === null) {
        targets = [];
        setTimeout(realMake, 100);
    }
    targets.push(target);
}
...
// Special targets to filter out
var specialTargets = [".PHONY", ".SUFFIXES", ".DEFAULT", ".PRECIOUS", ".SECONDARY", ".INTERMEDIATE", ".SECONDEXPANSION", ".DELETE_ON_ERROR", ".IGNORE", ".LOW_RESOLUTION_TIME", ".SILENT", ".EXPORT_ALL_VARIABLES", ".NOTPARALLEL", ".ONESHELL", ".POSIX"];
...
function filterTargets(targets, include, exclude) {
    ...
    if(include.length > 0) {
        ...
        for(var i in targets) {
            if(exclude.indexOf(i) == -1 && specialTargets.indexOf(i) == -1) {
                newTargets[i] = targets[i];
            }
        }
    }
